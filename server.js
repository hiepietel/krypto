...
DIEHARD = new Map();
DIEHARD.set("birthday-spacings-test",			"bin/dieharder -f {file} -d 0");
DIEHARD.set("overlapping-permutations-test",		"bin/dieharder -f {file} -d 1");
DIEHARD.set("ranks-of-matrices-test",			"bin/dieharder -f {file} -d 2");
DIEHARD.set("monkey-test",				"bin/dieharder -f {file} -d 4");
DIEHARD.set("count-1s-test",				"bin/dieharder -f {file} -d 8");
DIEHARD.set("parking-lot-test",				"bin/dieharder -f {file} -d 10");
DIEHARD.set("minimum-distance-test",			"bin/dieharder -f {file} -d 11");
DIEHARD.set("random-spheres-test",			"bin/dieharder -f {file} -d 12");
DIEHARD.set("squeeze-test",				"bin/dieharder -f {file} -d 13");
DIEHARD.set("overlapping-sums-test",			"bin/dieharder -f {file} -d 14");
DIEHARD.set("runs-test",				"bin/dieharder -f {file} -d 15");
DIEHARD.set("craps-test",				"bin/dieharder -f {file} -d 16");
ENT = new Map();
ENT.set("input-is-stream-byte-test", 			"bin/ent -b {file}");
ENT.set("occurences-count-test",			"bin/ent -c {file}" );
ENT.set("upper-to-lower-test", 				"bin/ent -f {file}");
ENT.set("csv-output-test", 				"bin/ent -t {file}");
ENT.set("help-test", 					"bin/ent -u {file}");
